...
describe("resolveTargetChainKey — chain conflict detection logic", () => {
  ...
  it("returns ethereum when only parsedChainId is Ethereum mainnet", () => {
    const result = resolveTargetChainKey({ parsedChainId: 1 });
    expect(result).toBe("ethereum");
  });

  it("returns base when only parsedChainId is Base mainnet", () => {
    const result = resolveTargetChainKey({ parsedChainId: 8453 });
