...
const testWalletHome = join(tmpdir(), "intentproof-test-wallets");
const requiredPolicyError =
  "analyze、sign、broadcast 皆必須提供 --policy（可為 JSON 檔案路徑或 inline JSON 字串，以 { 開頭自動判斷）。";
const requiredAnalyzeChainError =
  "缺少必要參數 --chain。合法值：eth-sepolia、base-sepolia、eth-mainnet、base-mainnet。";
const requiredTestnetChainError =
  "缺少必要參數 --chain。合法值：eth-sepolia、base-sepolia。";
const mainnetWriteBlocked =
  "is configured for decode/analyze and Protect Wallet forwarding only. sign remains testnet-only";
...
function runCli(args: string[]) {
  return spawnSync(
    process.execPath,
    ["--import", "tsx/esm", cliEntrypoint, ...args],
    {
      cwd: projectRoot,
      encoding: "utf8",
      env: { ...process.env, TOKENCORE_CLI_HOME: testWalletHome },
    },
  );
}
...
describe("cli policy gating", () => {
  const invalidPolicy =
    '{"version":1,"name":"bad","policies":[{"id":"bad","name":"bad","type":"unsupported"}]}';
  const sepoliaTransfer =
  ...
  it("fails closed before signing when policy precheck cannot be built", () => {
    const result = runCli([
      "sign",
      "--chain",
      "eth-sepolia",
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain("Policy 預檢失敗");
    expect(result.stderr).toContain("CLI 已停止 sign");
    expect(result.stderr).not.toContain("缺少必要參數 --wallet");
    expect(result.stderr).not.toContain("仍會繼續執行");
  });

  it("fails closed before broadcasting when policy precheck cannot be built", () => {
    const result = runCli([
      "broadcast",
      "--chain",
      "eth-sepolia",
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain("Policy 預檢失敗");
    expect(result.stderr).toContain("CLI 已停止 broadcast");
    expect(result.stderr).not.toContain("缺少必要參數 --wallet");
    expect(result.stderr).not.toContain("仍會繼續執行");
  });
});
...
describe("cli exit code contract", () => {
  ...
  it("returns 1 when analyze misses --chain", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain(requiredAnalyzeChainError);
  ...
  it("returns 1 when wallet create misses --chain", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain(requiredTestnetChainError);
...
describe("cli chain compatibility gating (P6-a)", () => {
  ...
  it("rejects mainnet signing while leaving mainnet analyze chain keys available", () => {
    const result = runCli([
      "sign",
      "--chain",
      "eth-mainnet",
      "--input",
      '{"chainId":1,"to":"0x1111111111111111111111111111111111111111","data":"0x","value":"0"}',
      "--policy",
      minimalPolicy,
      "--wallet",
      "missing-wallet",
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain(mainnetWriteBlocked);
    expect(result.stderr).toContain("eth-sepolia、base-sepolia");
  });
});
