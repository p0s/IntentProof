...
import { getChainConfig, getChainKeyById, getTokenPresets } from "./chains";
...
function parseOptionalBigInt(value: unknown, fieldName: string) {
  ...
  if (typeof value === "bigint") {
    if (value < 0n) throw new Error(`${fieldName} 必須是非負整數。`);
    return value;
  }
  if (typeof value === "number") {
    if (!Number.isSafeInteger(value) || value < 0) {
      throw new Error(`${fieldName} 必須是非負安全整數。`);
    }
    return BigInt(value);
  }
  ...
  throw new Error(`${fieldName} 必須是十進位或 0x 十六進位整數。`);
}
...
function parseOptionalNumber(
  ...
  const parsed = parseOptionalBigInt(value, fieldName);
  if (parsed === undefined) return undefined;
  if (parsed > BigInt(Number.MAX_SAFE_INTEGER) || parsed < BigInt(minimum)) {
    throw new Error(`${fieldName} 超出可支援範圍。`);
  }
  return Number(parsed);
}

function parseOptionalHex(value: unknown, fieldName: string) {
  if (value === undefined || value === null || value === "") return undefined;
  if (typeof value === "string" && /^0x[a-fA-F0-9]*$/.test(value)) {
    return value as Hex;
  }
  throw new Error(`${fieldName} 必須是 0x 開頭的十六進位字串。`);
}

function normalizeJsonInput(raw: string): ParsedInput {
  const parsed = JSON.parse(raw) as Record<string, unknown>;
  const chainId = parseOptionalNumber(parsed.chainId, "chainId", 1);
  ...
  return {
    ...
    data: parseOptionalHex(parsed.data, "data"),
    value: parseOptionalBigInt(parsed.value, "value"),
    nonce: parseOptionalNumber(parsed.nonce, "nonce"),
    gas: parseOptionalBigInt(gasValue, "gas"),
    raw,
  };
}
...
function buildKnownContractStatus(address: Address, chainKey: DemoChainKey) {
  ...
  const tokenPreset = getTokenPresets(chainKey).find(
    (token) => token.address.toLowerCase() === address.toLowerCase(),
  );
  if (tokenPreset) {
    const environmentLabel =
      chain.environment === "testnet" ? "testnet" : "mainnet";
    return {
      verified: true,
      source: "local",
      abi: commonAbiCandidates[0],
      contractName: tokenPreset.symbol,
      message: `命中內建 ${tokenPreset.symbol} ${environmentLabel} token 設定。`,
    } satisfies ContractVerificationStatus;
  }
...
function buildUnknownAction(
  ...
  return {
    ...
    decodeSource: signature ? "selector" : undefined,
  };
}
...
export async function decodeParsedInput(
  ...
  const registryEntry = lookupKeystoneAbiRegistry(
    getChainConfig(chainKey).chainId,
    parsed.to,
  );
  const candidateAbis: Array<{
    abi: Abi;
    source: "verified" | "registry" | "common";
  }> = [];
  if (verification.abi) {
    candidateAbis.push({ abi: verification.abi as Abi, source: "verified" });
  }
  if (registryEntry?.abi) {
    candidateAbis.push({ abi: registryEntry.abi, source: "registry" });
  }
  candidateAbis.push(
    ...commonAbiCandidates.map((abi) => ({
      abi: abi as Abi,
      source: "common" as const,
    })),
  );

  for (const candidate of candidateAbis) {
    const decoded = tryDecodeByAbi(candidate.abi, parsed.data);
    if (!decoded) continue;
