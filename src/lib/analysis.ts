...
export function composeEnglishSummary(
  chainLabel: string,
  action: DecodedAction,
  verification: ContractVerificationStatus,
  simulation: SimulationSummary,
  ...
  const actionName = action.functionName ?? action.title;
  const verificationSentence =
    action.kind === "nativeTransfer"
      ? "This is a native transfer and does not require contract verification."
      : verification.verified
        ? `The target contract is verified through ${formatVerificationSource(verification.source)}.`
        : "The target contract is not verified by the available sources.";
  const simulationSentence = simulation.success
    ? `Simulation preview is available from ${simulation.source}.`
    : `Simulation is unavailable or failed: ${simulation.errorMessage ?? "no reason provided"}.`;
  const policySentence =
    policyViolations.length > 0
      ? `${policyViolations.length} default policy issue(s) need review before signing.`
      : "No default policy violation is present in the decoded transaction.";

  return [
    `This transaction is prepared for ${chainLabel} and decodes as ${actionName}.`,
    verificationSentence,
    simulationSentence,
    policySentence,
  ].join(" ");
...
export function buildAnalysisResult(params: {
  ...
  const englishSummary = composeEnglishSummary(
    params.chainLabel,
    params.action,
    params.verification,
    params.simulation,
