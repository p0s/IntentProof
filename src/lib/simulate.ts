...
function getTenderlyPublicShareConfig() {
  ...
  const tenderlyApiAccessKey = getRuntimeEnv("TENDERLY_ACCESS_TOKEN");
  const tenderlyAccountSlug = getRuntimeEnv("TENDERLY_ACCOUNT_SLUG");
  const tenderlyProjectSlug = getRuntimeEnv("TENDERLY_PROJECT_SLUG");
...
// Used when REST API credentials (TENDERLY_ACCESS_TOKEN etc.) are absent
...
// ── Orchestrator ─────────────────────────────────────────────────────────────
// Priority 1: REST API simulation (server/CLI-only TENDERLY_ACCESS_TOKEN + slugs)
...
async function simulateWithTenderly(
  ...
  if (!rpcUrl) {
    return {
      ...
      errorMessage:
        "找不到 Tenderly 設定。請在 CLI/server 環境設定 TENDERLY_ACCESS_TOKEN、TENDERLY_ACCOUNT_SLUG、TENDERLY_PROJECT_SLUG（REST API），或在本機瀏覽器開發環境設定 VITE_TENDERLY_NODE_ACCESS_KEY（Node RPC）。",
      tokenChanges: fallbackChanges,
    };
  }
