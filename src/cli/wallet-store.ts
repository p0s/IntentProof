...
async function ensureWalletDirectory() {
  await mkdir(getCliWalletHome(), { recursive: true, mode: 0o700 });
  await chmod(getCliWalletHome(), 0o700);
  await mkdir(getCliWalletsDir(), { recursive: true, mode: 0o700 });
  await chmod(getCliWalletsDir(), 0o700);
}

function resolveCliHomeDir() {
  const configured = process.env.TOKENCORE_CLI_HOME?.trim();
  if (!configured) {
    return path.resolve(".tokencore-cli");
  }
...
export async function saveManagedWallet(wallet: StoredTokenCoreWallet) {
  ...
  await writeFile(filePath, serializeManagedWalletFile(wallet), {
    encoding: "utf8",
    mode: 0o600,
  });
