...
const supportedTestnetChainList = "eth-sepolia、base-sepolia";
const supportedAnalyzeChainList =
  "eth-sepolia、base-sepolia、eth-mainnet、base-mainnet";
...
用法：
  ...
  npm run cli -- analyze  --chain eth-sepolia|base-sepolia|eth-mainnet|base-mainnet --input '<tx-json|signed-rawtx>' --policy '<json-file|inline-json>' [--no-ai]
...
必填參數（analyze / sign / broadcast）：
  --chain <chain>                         analyze 可讀 mainnet；sign/broadcast 僅 testnet
...
function supportedChainListForCommand(command: ChainRequiredCommand) {
  return command === "analyze"
    ? supportedAnalyzeChainList
    : supportedTestnetChainList;
}
...
function buildMissingChainErrorMessage(command: ChainRequiredCommand) {
  ...
  return [
    `缺少必要參數 --chain。合法值：${supportedChainListForCommand(command)}。`,
    "範例：",
...
function assertTestnetOnlyChain(
  chainKey: DemoChainKey,
  operation: "wallet create" | "wallet import" | "sign" | "broadcast",
) {
  if (isTestnetChainKey(chainKey)) return;
  throw new Error(
    [
      `${getChainConfig(chainKey).label} is configured for decode/analyze and Protect Wallet forwarding only.`,
      `${operation} remains testnet-only in IntentProof Tx Guard.`,
      `Use one of: ${supportedTestnetChainList}.`,
    ].join(" "),
  );
}
...
function buildPolicyPrecheckFailureMessage(
  operation: "sign" | "broadcast",
  error: unknown,
) {
  return renderPolicyWarningText({
    operation,
    errorMessage:
...
async function handleAnalyze(args: ParsedArgs) {
  ...
  if (args.flags.has("no-ai")) {
    ...
    try {
      const aiResult = await generateAiSummary(analysis, { remote: true });
      if (aiResult) {
...
async function handleWalletCreate(args: ParsedArgs) {
  ...
  assertTestnetOnlyChain(chainKey, "wallet create");
...
async function handleWalletImport(args: ParsedArgs) {
  ...
  assertTestnetOnlyChain(chainKey, "wallet import");
  await initTokenCoreNodeWasm();
...
async function handleSign(args: ParsedArgs) {
  ...
  assertTestnetOnlyChain(explicitChainKey, "sign");
  assertChainCompatibility(explicitChainKey, parsed.chainId, "sign");

  const policyChainKey = explicitChainKey;
  let policyWarning = "";
  try {
    const analysis = await buildAnalysisWithPolicy(args, rawInput, policyChainKey);
    ...
    throw new Error(buildPolicyPrecheckFailureMessage("sign", error));
  }
...
async function handleBroadcast(args: ParsedArgs) {
  ...
  assertTestnetOnlyChain(explicitChainKey, "broadcast");
  ...
  if (parsed.type === "signedRaw") {
    ...
    try {
      ...
      throw new Error(buildPolicyPrecheckFailureMessage("broadcast", error));
    }
    try {
